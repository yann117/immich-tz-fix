...
export class PersonRepository {
  ...
  getAll(options: GetAllPeopleOptions = {}) {
    return this.db
      ...
      .$if(options.thumbnailPath !== undefined, (qb) =>
        qb.where('face_cluster.thumbnailPath', '=', options.thumbnailPath!),
      )
  ...
  getAllWithoutFaces() {
    return this.db
      ...
      .innerJoin('face_cluster', 'face_cluster.id', 'person.faceClusterId')
      .select('face_cluster.thumbnailPath')
      .where('asset_face.deletedAt', 'is', null)
      .where('asset_face.isVisible', 'is', true)
  ...
  getDistinctNames(userId: string, { withHidden }: PersonNameSearchOptions): Promise<PersonNameResponse[]> {
    return this.db
      .selectFrom('person')
      .innerJoin('face_cluster', 'face_cluster.id', 'person.faceClusterId')
      .select(['person.id', 'face_cluster.name'])
  ...
  getForPeopleDelete(ids: string[]) {
    ...
    return this.db
      .selectFrom('person')
      .innerJoin('face_cluster', 'face_cluster.id', 'person.faceClusterId')
      .select(['person.id', 'face_cluster.thumbnailPath'])
      .where('id', 'in', ids)
      .execute();
  }
